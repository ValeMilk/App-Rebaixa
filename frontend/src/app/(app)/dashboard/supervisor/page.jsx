      ...
      <div className="surface px-4 py-3">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex-1 relative">
            <IcoSearch className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" />
            <input
              type="text"
        ...
        {metricasFiltradas.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-12 text-center">
            <div className="w-16 h-16 rounded-full bg-neutral-100 flex items-center justify-center mx-auto mb-3">
              <IcoSearch className="w-8 h-8 text-neutral-400" />
            </div>
            <p className="text-neutral-600 font-medium">Nenhuma rede encontrada</p>
            <p className="text-sm text-neutral-400 mt-1">Tente ajustar os filtros de busca</p>
          </div>
        ) : (
  ...
  return (
    <div className={`bg-white rounded-lg border-l-[3px] ${status.border} border border-neutral-200 shadow-sm hover:shadow-lg hover:scale-[1.01] transition-all overflow-hidden`}>
      ...
      <div 
        ...
        className="cursor-pointer hover:bg-neutral-50/70 transition group"
      >
        <div className="px-4 py-2 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <button
              ...
              className="h-8 w-8 rounded-lg bg-neutral-100 hover:bg-neutral-200 flex items-center justify-center text-neutral-600 transition flex-shrink-0"
      ...
      {expandido && (
        <div className="border-t border-neutral-100 px-4 py-3 bg-neutral-50/50">
          ...
          {subredes?.length > 0 && (
            <div className="mb-3">
              <div className="flex items-center gap-2 mb-2">
                <IcoStore className="w-3.5 h-3.5 text-neutral-500" />
              ...
              <div className="flex flex-wrap gap-1.5">
                {subredes.map((s) => (
                  <span key={s} className="text-[11px] font-medium text-neutral-600 bg-white border border-neutral-200 rounded-full px-2.5 py-1">
                    {s}
                  </span>
                ))}
              </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <IcoPackage className="w-3.5 h-3.5 text-neutral-500" />
              <span className="text-xs font-semibold text-neutral-600 uppercase tracking-wide">Top Produtos</span>
            </div>
            {topProdutos.length === 0 ? (
              <p className="text-xs text-neutral-400 italic py-2">Nenhum produto encartado</p>
            ) : (
              <div className="space-y-1.5">
                {topProdutos.map((p, i) => (
                  <div key={i} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-neutral-200">
                    ...
                    <span className="ml-2 text-xs font-bold text-neutral-600 flex-shrink-0">{p.count}x</span>
                  </div>
                ))}
              </div>
            )}
