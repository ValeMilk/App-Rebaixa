    ...
    <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-5 flex flex-col gap-2">
      ...
      {v2 != null && (
        <div className="text-xs text-neutral-400">
          P2: <span className="font-semibold text-neutral-600">{fmt(v2)}</span>
        </div>
      )}
    </div>
  );
...
function TabelaSellout({ p1, p2 }) {
  ...
  if (!subs.length) return <p className="text-neutral-400 text-sm p-4">Nenhum dado de sellout encontrado.</p>;
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-neutral-50">
          {subs.map((s) => {
            ...
            return (
              <tr key={s.sub} className="hover:bg-neutral-50 transition">
                <td className="py-3 px-4 font-medium text-neutral-800">{s.sub}</td>
                <td className="py-3 px-4 text-right font-mono text-neutral-700">{fmtBRL(s.v1)}</td>
                {p2 && <td className="py-3 px-4 text-right font-mono text-neutral-700">{fmtBRL(s.v2)}</td>}
                {p2 && (
                ...
                <td className="py-3 px-4 text-right text-neutral-500">{s.n1 ?? s.n2 ?? 0}</td>
              </tr>
            );
          })}
        </tbody>
  ...
  function Th({ label, campo }) {
    ...
    return (
      <th
        ...
        className="text-right py-3 px-4 font-semibold cursor-pointer hover:text-neutral-800 select-none whitespace-nowrap"
      >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-neutral-50">
            {ordenados.map((enc, idx) => (
              <>
                <tr key={enc._id} className={`hover:bg-neutral-50 transition cursor-pointer ${expandido === enc._id ? "bg-blue-50" : ""}`} onClick={() => setExpandido(expandido === enc._id ? null : enc._id)}>
                  ...
                  <td className="py-3 px-4 text-neutral-600 text-xs">{enc.criadoPorNome}</td>
                  ...
                  <td className="py-3 px-4 text-right text-neutral-700">{enc.totalItens}</td>
                  <td className="py-3 px-4 text-right">
                    <span className={`font-bold text-sm ${corMargem(enc.margemMediaOferta)}`}>
                      {fmtPct(enc.margemMediaOferta)}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-right font-mono text-neutral-700">{fmtBRL(enc.precoMedioOferta)}</td>
                </tr>
                              ...
                              {(enc.subcategorias || []).length > 0 ? (
                                enc.subcategorias.map((sub) => (
                                  <tr key={sub.subcategoria} className="hover:bg-blue-100 transition">
                                    ...
                                    <td className="py-2 px-3 text-right font-mono text-neutral-700">{fmtBRL(sub.selloutMedio)}</td>
                                    <td className="py-2 px-3 text-center text-neutral-600">{sub.totalItens}</td>
                                  </tr>
                                ))
                              ) : (
                                <tr>
            ...
            {ordenados.length === 0 && (
              <tr><td colSpan={temP2 ? 11 : 10} className="py-8 text-center text-neutral-400">Nenhum encarte encontrado.</td></tr>
            )}
          </tbody>
        </table>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-4 space-y-4">
        <div className="flex flex-wrap items-end gap-4">
          ...
          {usarP2 && (
            <div>
              <label className="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-1">Período 2</label>
              <div className="flex items-center gap-2">
                <input
                  type="date"
                  value={p2inicio}
        ...
        <div className="flex flex-wrap items-end gap-4">
          {/* Filtro Rede */}
          <div>
            <label className="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-1">Rede</label>
            <select
              value={codigoRede}
          ...
          {supervisores.length > 0 && (
            <div>
              <label className="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-1">Supervisor</label>
              <select
                value={criadoPorId}
